...
//Dans le DOM un utilisateur peut supprimer attribut required des inputs => donc on check que c pas vide
        ...
        if (isset($_POST['email_utilisateur']) && !empty($_POST['email_utilisateur'])) {
            ...
            $this->email_user = trim(htmlspecialchars($_POST['email_utlisateur']));
        } else {
            //Sinon on affiche une erreur
            echo "<p class='alert-danger p-3'>Merci remplir le champ Email</p>";
        }

//  On fait la meme chose pour le mot de passe
...
//Si les 2 champs du formulaire ne sont pas vide
        if (!empty($_POST['email_user']) && !empty($_POST['password_user'])) {
            ...
            $sql = "SELECT * FROM utilisateur_location WHERE email = ? AND password = ?";
            ...
            if ($stmt->rowCount() >= 1) {
                ...
                //Si la prorpriété privée $email_user ($_POST['email_utilisateur']) est = a la données email de la table Utilisateurs ET

                //Si la prorpriété privée $password_user ($_POST['password_user']) est = a la données mot de passe de la table Utilisateurs
...
//var_dump($this->email_utilisateur);
