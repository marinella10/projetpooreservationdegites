...
class Administration extends Database
{
     ...
     * @var string
     */
    private $password_administration;
    ...
    public function connexionAdministration()
    {
        ...
        // On le fait aussi  pour le mot de passe

        if (isset($_POST['password_admin']) && !empty($_POST['password_admin'])) {
            $this->password_administration = trim(htmlspecialchars(strip_tags($_POST['password_admin'])));
        } else {
            echo "<p class='alert-danger p-3 mt-3'>Merci remplir le champ Email</p>";
        }
        ...
        if (!empty($_POST['email_admin']) && !empty($_POST['password_admin'])) {
            ...
            $sql = "SELECT * FROM administrateurs_loc WHERE email_admin = ? AND password_admin_vachar = ?";
            ...
            $stmt->bindParam(2, $this->password_administration);
            ...
            if ($stmt->rowCount() >= 1) {
                ...
                if ($this->email_admin === $row['email_admin'] && $this->password_administration === $row['password_admin_vachar']) {
                    //Demarre la seesion
                    session_start();
